...
class Blockchain {
   ...
   async generateGenesisBlock(){
       let self= this;
       try{
        if(await self.getBlockHeight() <= -1 ){ // Changed it to <= instead of == 
    ...
    async getBlockchain() {
        let self =this;
        let blockchain =await self.bd.getBlocksCount();
        // gets the blockchain 
    ...
    // this method returns the height of the last block
   async getBlockHeight() {
    ...
    //     let self =this;
    //     let blocks = await self.bd.getBlocksCount();
    //      for(var i=0 ; i<blocks.length ;i++){
    //          if(blocks[i].value.hash == hash){
    //              return blocks[i];
    //          }
    //      }
    ...
    //  }
     ...
     async getBlockByHash(hash) {
        try{
        let self= this;
        let value = await self.bd.getBlockByHash(hash);
        if (value != null){
   ...
   async getBlockByAddress(address) {
    try{
        let self= this;
        let value = await self.bd.getBlockByAddress(address);
        let blocks=[];
        if (value.length>0 ){
            for(var i=0 ; i<value.length;i++){
              let block= JSON.parse( value[i].value); 
              blocks.push(block);
       ...
       let self = this;
        ...
        try{
                ...
                if((await self.getBlockchainHeight()) >= 1){ 
                        let previousBlock = await self.getBlock(await self.getBlockchainHeight() - 1 ); // Fixed some Json.parse done
